...
const BUCKETS_RISK_LEVEL_TYPES_MAP = {
  critical: RiskLevelTypes.CRITICAL,
  high: RiskLevelTypes.HIGH,
  medium: RiskLevelTypes.MEDIUM,
}

const IdentityRiskAlerts: React.FC<ChartProps> = memo(({ globalTime, height }) => {
  ...
  const { data } = useStatefulApolloSubscription(BehavioralRiskLineQuery, {
    variables,
  ...
  const [hasValidData, chartData] = useMemo(() => {
    ...
    const chartData = data.behaviorRiskLine
      ...
      .map(series => {
        ...
        return {
          ...
          riskLevel,
          interval: data.behaviorRiskLine[0].data[1].time - data.behaviorRiskLine[0].data[0].time,
        }
      })
...
export default IdentityRiskAlerts
