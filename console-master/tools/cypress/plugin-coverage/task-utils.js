...
function checkAllPathsNotFound(nycFilename) {
  ...
  debug('in file %s all files are not found? %o', nycFilename, allFilesAreMissing)
...
function showNycInfo(nycFilename) {
  ...
  const coverageKeys = Object.keys(nycCoverage)
  if (!coverageKeys.length) {
    console.error('⚠️ file %s has no coverage information', chalk.yellow(nycFilename))
    console.error(
      'Did you forget to instrument your web application? Read %s',
      chalk.blue('https://github.com/cypress-io/code-coverage#instrument-your-application'),
    )
    return
  }
...
function resolveRelativePaths(nycFilename) {
  ...
  if (changed) {
    ...
    writeFileSync(nycFilename, JSON.stringify(nycCoverage, null, 2) + '\n', 'utf8')
  }
}
...
function findCommonRoot(filepaths) {
  ...
  for (let k = 0; k < shortestLength; k += 1) {
    ...
    const removedPrefixNames = filepaths.map(filepath => filepath.slice(prefix.length))
    debug('removedPrefix %o', removedPrefixNames)
    const foundAllPaths = removedPrefixNames.every(filepath => existsSync(join(cwd, filepath)))
...
function tryFindingLocalFiles(nycFilename) {
  ...
  const cwd = process.cwd()
  debug('found common folder %s that matches current working directory %s', commonFolder, cwd)
...
function findSourceFiles(nycOptions) {
  ...
  if (!Array.isArray(nycOptions.extension)) {
    console.error('Expected NYC "extension" option to be a list of file extensions')
  ...
  return globby.sync(patterns, { absolute: true })
}
/**
...
function includeAllFiles(nycFilename, nycOptions) {
  ...
  const nycCoverage = JSON.parse(readFileSync(nycFilename, 'utf8'))
  const coverageKeys = Object.keys(nycCoverage)
  const coveredPaths = coverageKeys.map(key => nycCoverage[key].path.replace(/\\/g, '/'))
