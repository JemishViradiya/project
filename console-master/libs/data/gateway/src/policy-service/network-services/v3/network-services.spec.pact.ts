...
import { networkServices, networkServicesMock } from './network-services-mock'

export const NETWORK_SERVICE_MOCK = networkServices[0]
  ...
  describe('get a list of network services', () => {
    beforeAll(() => {
      const interaction = new Interaction()
        ...
        .withRequest({
          method: 'GET',
          path: makeNetworkServicesEndpoint({ tenantId: TENANT_ID_MOCK }),
          headers: {
  ...
  describe('get a network service', () => {
    beforeAll(() => {
      const interaction = new Interaction()
        ...
        .withRequest({
          method: 'GET',
          path: makeNetworkServicesEndpoint({ tenantId: TENANT_ID_MOCK, networkServiceId: NETWORK_SERVICE_MOCK.id }),
          headers: {
            Authorization: Matchers.like(AUTHORIZATION_TOKEN_MOCK),
          },
        })
    ...
    it('should get a network service', async () => {
      const response = await NetworkServices.readOne(TENANT_ID_MOCK, NETWORK_SERVICE_MOCK.id)
