...
import { ApprovedApps as ApprovedAPPs, MobileProtectData } from '@ues-data/mtd'
import {
  FeatureName,
  Permission,
  useFeatures,
...
const defaultDialogState: AppModalDialogProps = {
  ...
  onFormSubmit: (approvedApp: IAppInfo) => {
    return
  },
}
...
const ApprovedApps: React.FC = () => {
  ...
  const isIosSafeAppsEnabled = isEnabled(FeatureName.ExclusionIosSafeApps)
  ...
  const { data, refetch, fetchMore } = useStatefulReduxQuery(ApprovedAPPs.queryApprovedApps, initialSearchQueryParams)
  const appDataTask = useSelector(ApprovedAPPs.getApprovedAppsTask)

  // Edit application functionality below
  const [editApprovedApplicationStartAction, editApprovedAppTask] = useStatefulReduxMutation(
    ApprovedAPPs.mutationEditApprovedApplication,
  )
  const editApprovedAppTaskPrev = usePrevious(editApprovedAppTask)
  useEffect(() => {
    if (!editApprovedAppTask.loading && editApprovedAppTaskPrev.loading && editApprovedAppTask.error) {
      snackbar.enqueueMessage(t('exclusion.apps.applicationEditErrorMsg'), 'error')
    } else if (!editApprovedAppTask.loading && editApprovedAppTaskPrev.loading) {
  ...
  const openForEdit = useCallback(
    (appToEdit: IAppInfo): boolean => {
      setDialogProps({
        inputValue: appToEdit,
        iosEnabled: isIosSafeAppsEnabled,
        openDialog: true,
        onClose: () => {
          setDialogProps(defaultDialogState)
        },
        onFormSubmit: (approvedApp: IAppInfo) => {
          editApprovedApplicationStartAction(approvedApp)
        },
        headerTitle: t('exclusion.approvedApps.editFormName'),
        submitBtnTitle: t('common.save'),
        isEditMode: true,
      })
      return false
    },
    [editApprovedApplicationStartAction, t, isIosSafeAppsEnabled],
  )
  ...
  const { showDeleteConfirmation } = useMultipleDelete(
    ...
    getSelected,
    refetch,
    ApprovedAPPs.mutationDeleteApprovedApplications,
    'APP',
    idFunction,
  )

  //Create application functionality below
  const [createApprovedApplicationStartAction, createApprovedAppTask] = useStatefulReduxMutation(
    ApprovedAPPs.mutationCreateApprovedApplication,
  )
  const createApprovedAppTaskPrev = usePrevious(createApprovedAppTask)
  useEffect(() => {
    if (!createApprovedAppTask.loading && createApprovedAppTaskPrev.loading && createApprovedAppTask.error) {
      if ((createApprovedAppTask as any).error.response?.data?.subStatusCode === 210) {
    ...
    } else if (!createApprovedAppTask.loading && createApprovedAppTaskPrev.loading) {
      setDialogProps(defaultDialogState) // clear form and close dialog
  ...
  const openAddManualForm = (predefinedAppInfo: IAppInfo = null) => {
    setDialogProps({
      ...
      onFormSubmit: (approvedApp: IAppInfo) => {
        createApprovedApplicationStartAction(approvedApp)
      },
      headerTitle: t('exclusion.approvedApps.addFormName'),
  ...
  const [importAppsStartAction, importAppsTask] = useStatefulReduxMutation(ApprovedAPPs.mutationImportApprovedApplications)
  const importAppsTaskPrev = usePrevious(importAppsTask)
  useEffect(() => {
    if (!importAppsTask.loading && importAppsTaskPrev.loading && importAppsTask.error) {
      setImportLoading(false)
      importOnClose()
    ...
    } else if (!importAppsTask.loading && importAppsTaskPrev.loading) {
      setImportLoading(false)
      importOnClose()
