...
import { securityRiskCategoriesWidgetMock } from './mock'
import { querySecurityRiskCategoriesGql } from './query'

const COMPONENT_NAME = 'dashboard/security-risk-categories-widget'
  ...
  describe(`get ${COMPONENT_NAME} data`, () => {
    ...
    beforeAll(() => {
      const interaction = new GraphQLInteraction()
        ...
        .withOperation('querySecurityRiskCategories')
        .withQuery(print(querySecurityRiskCategoriesGql))
        ...
        .willRespondWith({
          ...
          body: { data: Matchers.like(securityRiskCategoriesWidgetMock) },
        })
    ...
    it(`should get ${COMPONENT_NAME} data`, async () => {
      ...
      expect(response.data).toEqual(securityRiskCategoriesWidgetMock)
    })
  })
})
