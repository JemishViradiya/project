...
import { ApprovedDevCerts, MobileProtectData } from '@ues-data/mtd'
...
const defaultDialogState: DevCertModalDialogProps = {
  ...
  onFormSubmit: (devCert: IDeveloperCertificate) => {
    return
  },
}

// eslint-disable-next-line sonarjs/cognitive-complexity
const ApprovedDevelopers = memo(() => {
  ...
  const { refetch, fetchMore } = useStatefulReduxQuery(ApprovedDevCerts.queryApprovedDevCerts, initialSearchQueryParams)
  const devCertsTask = useSelector(ApprovedDevCerts.getApprovedDevCertsTask)

  // Edit developer certificate functionality below
  const [editApprovedDevCertStartAction, editApprovedDevCertTask] = useStatefulReduxMutation(
    ApprovedDevCerts.mutationEditApprovedDevCert,
  )
  const editApprovedDevCertTaskPrev = usePrevious(editApprovedDevCertTask)
  useEffect(() => {
    if (!editApprovedDevCertTask.loading && editApprovedDevCertTaskPrev.loading && editApprovedDevCertTask.error) {
      if (editApprovedDevCertTask.error['response'].status === 409) {
    ...
    } else if (!editApprovedDevCertTask.loading && editApprovedDevCertTaskPrev.loading) {
  ...
  }, [editApprovedDevCertTask])
  const openForEdit = (devCertToEdit: IDeveloperCertificate): boolean => {
    setDialogProps({
      ...
      onFormSubmit: (devCert: IDeveloperCertificate) => {
        editApprovedDevCertStartAction(devCert)
      },
      headerTitle: t('exclusion.approvedDevelopers.editFormName'),
  ...
  const { showDeleteConfirmation } = useMultipleDelete(
    ...
    refetch,
    ApprovedDevCerts.mutationDeleteApprovedDevCerts,
    'DEVCERT',
    idFunction,
  ...
  const [createDevCertStartAction, createDevCertTask] = useStatefulReduxMutation(ApprovedDevCerts.mutationCreateApprovedDevCert)
  ...
  const openAddManualForm = (predefinedDevCertInfo: IDeveloperCertificate = null) => {
    setDialogProps({
      ...
      headerTitle: t('exclusion.approvedDevelopers.addFormName'),
  ...
  const [importDevCertsStartAction, importDevCertsTask] = useStatefulReduxMutation(ApprovedDevCerts.mutationImportApprovedDevCerts)
  ...
  const toolbarProps = useExclusionsListToolbar({
    ...
    exportFileNamePrefix: 'SafeDevelopers',
...
export default ApprovedDevelopers
