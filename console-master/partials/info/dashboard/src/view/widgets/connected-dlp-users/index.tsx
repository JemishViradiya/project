...
const ConnectedDlpUsersWidget: React.FC<ChartProps> = ({ globalTime }) => {
  const { t } = useTranslation('dlp/common')
  const classes = useStyles()

  const { error, loading, data } = useStatefulReduxQuery(DashboardData.queryNumberActiveUsers)
  ...
  return (
    <Box height="100%" display="flex" pt={2} justifyContent="center" className={classes.totalCountWrapper}>
      <TotalCount count={String(connectedUsersCount?.activeUsers ?? 0)} description={t('dashboard.connectedUsers')} />
    </Box>
  )
}

export default ConnectedDlpUsersWidget
