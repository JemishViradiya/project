...
const TABLE_NAME = 'mobileAlertsByUser'
const FILTERED_TABLES = []

export const MobileAlertByUser = React.memo(() => {
  ...
  const fixedFilters: Record<string, string> = React.useMemo(() => {
    if (params?.id) {
      // interrogate path to obtain ecoId data filter
      return { [MobileAlertFilterNames.EcoId]: atob(params.id) }
    }
    return {}
  }, [params.id])
