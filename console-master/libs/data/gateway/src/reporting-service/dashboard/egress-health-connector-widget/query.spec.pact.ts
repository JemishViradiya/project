...
import { egressHealthConnectorMock } from './mock'
import { queryEgressHealthConnectorGql } from './query'

const COMPONENT_NAME = 'dashboard/egress-health-connector-widget'
  ...
  describe(`get ${COMPONENT_NAME} data`, () => {
    ...
    beforeAll(() => {
      const interaction = new GraphQLInteraction()
        ...
        .withOperation('queryEgressHealthConnector')
        .withQuery(print(queryEgressHealthConnectorGql))
        ...
        .willRespondWith({
          ...
          body: { data: Matchers.like(egressHealthConnectorMock) },
        })
    ...
    it(`should get ${COMPONENT_NAME} data`, async () => {
      ...
      expect(response.data).toEqual(egressHealthConnectorMock)
    })
  })
})
