...
export const ArrState = memo<{
  datapointId?: string
  state?: ChallengeState
  currentRiskLevel: string
  displayRiskLevelChange: boolean
}>(({ datapointId, state, currentRiskLevel, displayRiskLevelChange }) => {
  ...
  const entries = useMemo(
    () =>
      (data?.fixupDetails ?? []).filter(
        entry => EXPECTED_CHALLENGE_STATES_SET.has(entry.state) || (displayRiskLevelChange && entry.previousBehavioralRiskLevel),
      ),
    [data?.fixupDetails, displayRiskLevelChange],
  )
        ...
        <Collapse className={classNames.detailsContainer} in={!collapsed}>
          {entries.map(entry => (
            <div className={classNames.detailsEntry}>
              <time className={classNames.timeContainer}>{i18n.format(entry.datetime, I18nFormats.DateTime)}</time>
              {/* displayRiskLevelChange is only for now when Risk is hidden for all users */}
              {entry.previousBehavioralRiskLevel && displayRiskLevelChange ? (
                <span className={classNames.statusMargin}>
                  {t('bis/ues:risk.common.riskChanged', {
                    prevLevel: t(`bis/shared:risk.level.${entry.previousBehavioralRiskLevel}`),
                    currentLevel: t(`bis/shared:risk.level.${currentRiskLevel}`),
                  })}
                </span>
              ) : (
                <span className={classNames.statusMargin}>{t(`bis/shared:arr.challengeState.${entry.state}`)}</span>
              )}
            </div>
          ))}
        </Collapse>
