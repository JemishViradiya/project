// TODO: separate all the loading of the map api from the drawing classes
import styles from './DefaultPopup.module.less'
import { hasLoaded } from './loadGoogleApi'

// argh. we can't really build until maps is loaded
// but we're not going to know.  So, you as the user
// have to ensure that the maps api is there before
// you use this class...

// keep track of which popup, if any, is active on
// any given map.
export const activePopup = new Map() // exported for testing purposes only
const ACTION_ITEMS_HEIGHT = 120
...
export const Popup = function () {
  if (!window.google) return null // sigh...
...
// we can only finish this once the API is loaded
const finalizePopupClass = google => {
  ...
  Popup.prototype.setPosition = function (position, invertedPosition = position) {
    ...
    if (this.invertedPosition) {
      this.containerDiv.classList.add(styles.positioned)
    }
  }

  Popup.prototype.setRisk = function (value) {
    this.bubbleAnchor.setAttribute('risk', value)
  }

  Popup.prototype.setMarker = function (marker) {
    this.marker = marker
    this.containerDiv.classList.remove(styles.positioned)
  }
  ...
  Popup.prototype.setPrivate = function (privacyModeOn) {
  ...
  Popup.prototype.draw = function () {
    ...
    if (containerPosition.y - this.popupHeight < ACTION_ITEMS_HEIGHT) {
      if (this.invertedPosition) {
        divPosition = this.getProjection().fromLatLngToDivPixel(this.invertedPosition)
      }
      if (!this.inverted) {
        this.inverted = true
        this.containerDiv.classList.add(styles.inverted)
      }
    } else if (this.inverted) {
      this.inverted = false
      this.containerDiv.classList.remove(styles.inverted)
    }

    if (this.privacyModeOn) {
      this.containerDiv.classList.add(styles.privacyModeOn)
    } else {
      this.containerDiv.classList.remove(styles.privacyModeOn)
    }
...
// finish setting up the Popup class if or when we can
if (window.google && window.google.maps && window.google.maps.Map) {
  finalizePopupClass(window.google)
} else {
  // argh. have to wait for it.
  hasLoaded().then(finalizePopupClass)
}
