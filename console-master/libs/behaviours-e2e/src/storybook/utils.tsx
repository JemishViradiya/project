...
function StorybookWorkerLoader({ children }): JSX.Element {
  usePromise(() => import('@ues/service-worker'), [])
  console.log('StorybookWorkerLoader: [loaded]', !!window['workbox'])
  return <>{React.Children.toArray(children)}</>
}

export const useServiceWorkerDecorator = (element: JSX.Element): JSX.Element => {
  Object.assign(React.Suspense, { displayName: 'React.Suspense' })
  return (
    <React.Suspense fallback={'Installing Service-Worker'}>
      <StorybookWorkerLoader>{element}</StorybookWorkerLoader>
    </React.Suspense>
  )
}
